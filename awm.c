...
void tile()
{
	...
	for (c = clients; c; c = c->next) {
		if (c->tags & sel_tag) {	/* si la ventana pertenece al tag actual */
			...
			//int border = 2;
			XMoveResizeWindow(d, c->win, x, y, w - (border * 2),
					  h - (border * 2));
			XSetWindowBorderWidth(d, c->win, border);
			XSetWindowBorder(d, c->win, 0xff44444);
			XMapWindow(d, c->win);
...
void focusstack(const Arg *arg)
{
	...
	tile();
	XSetInputFocus(d, clients->win, RevertToParent, CurrentTime);
	XRaiseWindow(d, clients->win);
}
...
//      Window root;
	...
	while (!XNextEvent(d, &ev)) {
		if (ev.type == KeyPress) {
			for (int i = 0; i < sizeof(keys) / sizeof(keys[0]); i++) {
				if (ev.xkey.keycode ==
				    XKeysymToKeycode(d, keys[i].keysym)
			...
			XConfigureWindow(d, ev.xmaprequest.window,
					 ...
					 CWBorderWidth, &wc);
			XSetWindowBorder(d, ev.xmaprequest.window, 0xff0000);
			XMapWindow(d, ev.xmaprequest.window);
